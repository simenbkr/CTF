...
/*   #include <sys/commifmt.h> */
   #include <fcntl.h>
   #include <proto/dos.h>	/* for SetFileDate() */
                            ...
                            )
{
   ...
   for (i = 0; i <= 255; i++)	/* 'yy' initialised here! */
      yy[i] = (UChar) i;
   ...
   for (i = 0; i <= last; i++) {
      ...
      j = 0;
      tmp = yy[j];	/* IGNORE 'yy' uninitialised warning here */
      while ( ll_i != tmp ) {
         j++;
         tmp2 = tmp;
         tmp = yy[j];
...
void sortIt ( void )
{
   ...
   if (lastPP <= 1024) {
      ...
      for (grade = 1; grade <= 5; grade++) {
         Int32 candNo;
         /* fixed potential bug, cleared loBound and hiBound (R.Perrott 10/02/1997) */
         Int32 loBound = 0;	
		 Int32 hiBound = 0;
...
void copyDateAndPermissions ( Char *srcName, Char *dstName )
{
   ...
   retVal = SetFileDate( dstName,__timecvt(statBuf.st_mtime) );
   ERROR_IF_NOT_ZERO ( retVal );
}
#else
/*---------------------------------------------*/
void copyDateAndPermissions ( Char *srcName, Char *dstName )
{
   IntNative      retVal;
   struct MY_STAT statBuf;
   struct utimbuf uTimBuf;	/* actime, modtime */
...
{
   /* fixed potential bug, put NULL in FILE handles (R.Perrott 10/02/1997) */
   FILE *inStr = NULL;
   FILE *outStr = NULL;
...
IntNative main ( IntNative argc, Char *argv[] )
{
   ...
   #if _AMIGA
   /* cast to stop type errors */
   signal (SIGINT,  (void *)(int)mySignalCatcher);
   signal (SIGTERM, (void *)(int)mySignalCatcher);
   signal (SIGSEGV, (void *)(int)mySIGSEGVorSIGBUScatcher);
   #else
